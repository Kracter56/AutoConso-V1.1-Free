 ...
 */
    ...
    func tableView(_ tableView: UITableView, editActionsForRowAt indexPath: IndexPath) -> [UITableViewRowAction]? {
        let deleteAction = UITableViewRowAction(style: .default, title: "Supprimer") { (deleteAction, indexPath) -> Void in
            ...
            let titleDeleteCar = NSLocalizedString("Suppression véhicule", comment: "titleDeleteCar")
			let messageDeleteCar = NSLocalizedString("Etes-vous sûr de vouloir supprimer le véhicule sélectionné ?", comment: "messageDeleteCar")
			let deleteTitle = NSLocalizedString("Supprimé !", comment: "confirmDelete")
			let deleteMessage = NSLocalizedString("Le véhicule a bien été supprimée de la base", comment: "deleteMessage")
			...
			SweetAlert().showAlert(titleDeleteCar, subTitle: messageDeleteCar, style: AlertStyle.warning, buttonTitle:textNo, buttonColor: self.UIColorFromRGB(rgbValue: 0xD0D0D0) , otherButtonTitle:  textYes, otherButtonColor: self.UIColorFromRGB(rgbValue: 0xDD6B55)) { (isOtherButton) -> Void in
				if isOtherButton == true {
					...
					try! self.realm!.write{
						...
						print("Le véhicule a bien été supprimée de la base")
						self.listCars()
					}
					SweetAlert().showAlert(deleteTitle, subTitle: deleteMessage, style: AlertStyle.success)
				}
			}
        }
        return [deleteAction]
...
extension UITableView {
    ...
    func setEmptyMessage(_ message: String) {
		let messageLabel = UILabel(frame: CGRect(x: 0.0, y: 0.0, width: self.bounds.size.width, height: self.bounds.size.height))
