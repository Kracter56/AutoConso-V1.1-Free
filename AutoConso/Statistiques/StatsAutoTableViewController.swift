...
class StatsAutoTableViewController: UITableViewController {
	...
	override func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
		...
		let vc = storyboard.instantiateViewController(withIdentifier: "statsChartsScreen") as! GraphesViewController
		//let vc = GraphesViewController()
		switch indexPath.section {
		case 0:
			switch indexPath.row {
			case 0:
				// Afficher les différents kilométrages véhicule en fonction de la date
				vc.idCar = self.idCar
				vc.objToDisplay = "km"
				break
			default:
